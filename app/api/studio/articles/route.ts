...
async function attachSources(client: any, articleId: string, sources: Array<{url:string;note:string|null}>) {
  ...
  for (const item of sources) {
    ...
    const { data: source } = await client.from("sources").insert({
      name: hostname, url: item.url, source_type: "web",
    }).select("id").single();
    if (source?.id) {
      await client.from("article_sources").insert({
        article_id: articleId, source_id: source.id, source_url: item.url,
        note: item.note, verified: true,
      });
    }
  }
}
...
export async function GET(request: Request) {
  try {
    ...
    let query = session.client!.from("articles")
      .select("id,slug,title,dek,section,status,story_type,updated_at,published_at,scheduled_at,created_by")
      .order("updated_at",{ascending:false}).limit(100);

    if (session.profile?.role === "author") query = query.eq("created_by", session.user!.id);
