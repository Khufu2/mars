...
export default function SocialStudio({ searchParams }: { searchParams?: { slug?: string } }) {
  const article = articles.find(item => item.slug === searchParams?.slug) || articles[0];
  ...
  return (
    <main className="studioPage">
      ...
      <div className="socialChooser">
        {articles.map(item => <Link className={item.slug === article.slug ? "active" : ""} href={"/studio/social?slug=" + item.slug} key={item.slug}>{item.section}: {item.title}</Link>)}
      </div>
      ...
      <section className="captionBox">
        <span className="miniLabel">CAPTION DRAFT</span>
        <pre>{caption}</pre>
      </section>
    </main>
  );
